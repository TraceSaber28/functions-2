...
const applyDiscount = (arrayName, discount, callbackName) => {
    arrayName.forEach((element) => callbackName(element, discount))
}

applyDiscount(dogProducts, 5, applyFlateRateDiscount)
console.log(dogProducts);
applyDiscount(catProducts, .25, applyPercentDiscount);
...
const placesThatStartWithA  = places.filter((element) => element.startsWith('A'));
...
const remaining  = expenses.reduce((accumulator, current) => accumulator - current.amount, budget);
